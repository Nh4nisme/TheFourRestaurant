...
public class GiaoDienLapHoaDon extends VBox {
	...
	private void khoiTaoGiaoDien() {
		...
		thongTin.setStyle(
				"-fx-background-color: #F6F6F6; -fx-border-color: #DDB248; -fx-border-radius: 8; -fx-background-radius: 8;");
		...
		btnKiemTraKM.setStyle(
				"-fx-background-color: #1E424D; -fx-text-fill: #DDB248; -fx-font-weight: bold; -fx-cursor: hand;");
		...
		ImageView qrView = new ImageView(
				new Image(getClass().getResourceAsStream("/com/thefourrestaurant/images/QR.png")));
	...
	private void khoiTaoBangMon() {
		...
		colMaMon.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().getMonAn().getMaMonAn()));
		colMaMon.setPrefWidth(100);

		// Cột Tên món
		TableColumn<ChiTietPDB, String> colTenMon = new TableColumn<>("Tên món");
		colTenMon.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().getMonAn().getTenMon()));
		...
		colDonGia.setCellValueFactory(
				data -> new SimpleObjectProperty<>(BigDecimal.valueOf(data.getValue().getDonGia())));
		...
		colThanhTien.setCellValueFactory(data -> {
			BigDecimal thanhTien = BigDecimal.valueOf(data.getValue().getDonGia())
	...
	private void taoHoaDonMoi() {
		try {
			...
			if (!inputKM.isEmpty()) {
				...
				if (km != null) {
					...
					if (km.getTyLe() != null) { // giảm theo %
						BigDecimal chietKhau = thanhToan.multiply(km.getTyLe()).divide(BigDecimal.valueOf(100), 2,
			...
			BigDecimal vat = thanhToan.multiply(BigDecimal.valueOf(0.1)).setScale(0, RoundingMode.HALF_UP);
			thanhToan = thanhToan.add(vat);
			...
			if (tienThua.compareTo(BigDecimal.ZERO) < 0) {
				thongBao("Tiền khách đưa không đủ để thanh toán!\nTiền cần: " + formatTien(thanhToan)
						+ " đ, khách đưa: " + formatTien(tienKhachDua) + " đ", Alert.AlertType.WARNING);
				return;
			}
			...
			thongBao("Tạo hóa đơn thành công!\nTổng tiền phải trả sau trừ cọc: " + formatTien(thanhToan) + " đ",
