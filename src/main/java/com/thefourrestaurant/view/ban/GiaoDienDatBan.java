...
public class GiaoDienDatBan extends BorderPane {
	...
	private VBox taoThanhBen() {
		...
		thanhBen.getChildren().addAll(taoHeaderThanhBen(), taoNutChucNang(), taoSpacer(), taoChuThichThanhBen());
	...
	private VBox taoThanhDieuHuong() {
		...
		thanhDieuHuong.setStyle("-fx-background-color: " + COLOR_BACKGROUND_SIDE + ";"
	...
	private Label taoLabel(String text, int fontSize, boolean bold) {
		...
		lbl.setStyle("-fx-text-fill: " + COLOR_TEXT + ";" + (bold ? "-fx-font-weight: bold;" : "") + "-fx-font-size: "
				+ fontSize + "px;");
		return lbl;
	}
